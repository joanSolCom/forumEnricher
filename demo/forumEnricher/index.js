...
$( document ).ready(function() {
	...
	**/
	...
	$( "button.sentences" ).click(function()
	{
		...
		$.getJSON( "http://localhost:5000/getSentences?callback=?", { "idFile": idFile, "idFileEnriched":idFileEnriched} ,function( data ) {
			var stringVal = "";
			for(var i in data){
				stringVal+= " [" + data[i] + "]<br/>";
			}
	...
	$( "button.postFeatures" ).click(function() 
	{
		...
		else
		{
			...
			$.getJSON( "http://localhost:5000/getPostFeatures?callback=?", { "idFile": idFile, "idPost":idPost} ,function( data ) {
				var stringVal = "";
				for(var i in data)
				{
					stringVal+= data[i][0] + " " + data[i][1]+"<br/>";
	...
	$( "#showUserRoles" ).click(function() {
		...
		else
		{
			if($("#showUserRoles").text() == "Show User Roles")
			{
				...
				$("#userRoles").show();
				/*
				$.getJSON( "http://localhost:5000/getUserRoles?callback=?", { "idFile": idFile} ,function( data ) {
